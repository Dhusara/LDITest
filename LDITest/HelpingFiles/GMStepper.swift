...
@IBDesignable public class GMStepper: UIControl {
    ...
    @objc @IBInspectable public var value: Double = 0 {
        didSet {
            ...
            if oldValue != value { sendActions(for: .valueChanged) }
        }
    }
    ...
    private var formattedValue: String? {
        ...
        } else { return formatter.string(from: NSNumber(value: value)) }
    }
    ...
    @objc public var buttonsFont = UIFont(name: "AvenirNext-Bold", size: 20.0)! {
        didSet {
            for button in [leftButton, rightButton] { button.titleLabel?.font = buttonsFont }
        }
    }
...
extension GMStepper {
    @objc func handleTimerFire(timer: Timer) {
        timerFireCount += 1
        if timerFireCount % timerFireCountModulo == 0 { updateValue() }
    }
